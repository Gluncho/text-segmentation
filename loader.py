...
def load_model_and_tokenizer(pretrained_dir: str, device: str = DEFAULT_DEVICE):
    ...
    doc_encoder = AutoModelForTokenClassification.from_pretrained(f"{pretrained_dir}/doc_encoder")
    model = MiniSeg(sentence_encoder, doc_encoder).to(device)
    model.load_state_dict(torch.load(f"{pretrained_dir}/miniseg_model.pth", map_location=device))
    model.eval()
